...
class JunitReporter extends events.EventEmitter {
    ...
    onSingleFileOutput () {
        const runners = this.baseReporter.stats.runners
        const { xml, testResults } = this.prepareTestResults(runners)
        const runner = runners[Object.keys(runners)[0]]
        const browserName = runner.capabilities.browserName.split(' ').join('')
        const testPlanKey = this.config.suite
        const testResultsStr = `P${testResults.pass} F${testResults.fail} S${testResults.pending}`
        const now = moment().format('DD.MM.YYYY HH-mm-ss')
        let filename = `${testPlanKey} ${browserName} [${testResultsStr}] [${now}].xml` // default
    ...
    onMultiFileOutput () {
        for (let cid of Object.keys(this.baseReporter.stats.runners)) {
            ...
            const { xml } = this.prepareTestResults(singleRunner)
                    ...
                    for (let testKey of Object.keys(suite.tests)) {
                        if (testKey !== 'undefined') { // fix cucumber hooks crashing reporter
                            ...
                            let testName,
                                ...
                                testPriority
                            try {
                                const json = JSON.parse(test.title)
                                testName = json.name
                                testDesc = json.desc
